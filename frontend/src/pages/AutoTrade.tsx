...
import { terminalApi, type AutoTradeConfig, type AutoTradeRunnerStatus, type ExecutionLog, type McpToken, type Mt5Bar, type Mt5ConfigurationInput, type Mt5LiveSnapshot, type Mt5Position } from "@/lib/trading-terminal-api";
  ...
  return <div className="min-h-screen bg-[#080b12] p-3 font-mono text-slate-200 lg:h-screen lg:overflow-hidden">
    <div className="grid min-h-[calc(100vh-24px)] gap-3 lg:h-[calc(100vh-24px)] lg:grid-cols-[230px_minmax(420px,1fr)_320px]">
      ...
      <main className="grid min-h-0 gap-3 lg:grid-rows-[minmax(300px,1.4fr)_minmax(220px,1fr)_150px]">
        ...
        <section className="terminal-panel p-4"><h2 className="terminal-title">AI SIGNAL & MATH CALC</h2><div className="mt-3 flex items-end justify-between"><strong className={`text-4xl ${runnerStatus?.lastDecision === "BUY" ? "text-emerald-300" : runnerStatus?.lastDecision === "SELL" ? "text-rose-400" : "text-slate-400"}`}>{runnerStatus?.lastDecision || "HOLD"}</strong><span className="text-xs text-slate-400">Engine: {runnerStatus?.state === "RUNNING" ? "LIVE" : runnerStatus?.state === "ERROR" ? "ERROR" : "STANDBY"}</span></div><div className="mt-4 grid grid-cols-4 gap-4 text-xs"><Quote label="Entry" value={signal?.price} /><Quote label="SL" value={signal?.stopLoss} danger /><Quote label="TP" value={signal?.takeProfit} positive /><Quote label="Order" value={runnerStatus?.lastOrderId ? Number(runnerStatus.lastOrderId) : null} /></div><pre className="terminal-inset mt-4 max-h-28 overflow-auto whitespace-pre-wrap p-3 text-[10px] leading-5 text-slate-400">{`STRATEGY: EMA 9/21 crossover (closed candle)\nMODE: Demo only Â· spread guard ON Â· max 1 posisi\nSTATE: ${runnerStatus?.state || "IDLE"}\nLAST CANDLE: ${runnerStatus?.lastCandleAt || "-"}\nDECISION: ${runnerStatus?.lastDecision || "-"}\nMESSAGE: ${runnerStatus?.message || "Tekan START untuk mulai."}${runnerStatus?.lastError ? "\nERROR: " + runnerStatus.lastError : ""}`}</pre></section>
